...
class ConfluenceOptimizedMCP {
  constructor() {
    ...
    this.regexPatterns = {
      ...
      metaTags: /<(?:script|style|noscript|meta|link)[^>]*>.*?<\/(?:script|style|noscript|meta|link)>/gis,
      selfClosingMeta: /<(?:script|style|noscript|meta|link)[^>]*\/>/gi,
      multimedia: /<(?:iframe|embed|object|canvas|svg|audio|video)[^>]*>.*?<\/(?:iframe|embed|object|canvas|svg|audio|video)>/gis,
      selfClosingMultimedia: /<(?:iframe|embed|object|canvas|svg|audio|video)[^>]*\/>/gi,
      comments: /<!--.*?-->/gis,
      confluenceClasses: /<div[^>]*class="[^"]*confluence[^"]*"[^>]*>.*?<\/div>/gis,
    ...
    this.htmlToTextOptions = {
      ...
      ignoreImage: true,
      selectors: [
        // Confluence 특수 태그들 완전 제거
        { selector: 'ac\\:structured-macro', format: 'skip' },
        ...
        { selector: 'ri\\:page', format: 'skip' },
        { selector: 'ri\\:user', format: 'skip' },
  ...
  // HTML 전처리 - 불필요한 요소들을 미리 제거 (최적화된 버전)
  preprocessHtml(html) {
    if (!html) return '';

    let cleaned = html;

    // 미리 컴파일된 정규식 사용으로 성능 최적화
    cleaned = cleaned.replace(this.regexPatterns.confluenceTags, '');
  ...
  cleanHtmlToText(html) {
    ...
    try {
      ...
      return this.preprocessHtml(html).replace(/<[^>]*>/g, '').trim();
    }
  }
  ...
  optimizeResponse(data, options = {}) {
    ...
    if (typeof data === 'object') {
      ...
      if (data.version?.number) {
        optimized.version = data.version.number;
      }
  ...
  async getPage(pageId) {
    try {
      const result = await this.makeRequest(`/content/${pageId}?expand=body.storage,version`);
  ...
  async searchPagesCompact(query, limit = 10) {
    try {
      ...
      if (result.results) {
        result.results = result.results.map(item =>
          this.optimizeResponse(item, {
            fields: ['id', 'title', 'type', 'status', 'space'],
            includeBody: false
          })
        );
      }
  ...
  async getPageComments(pageId, limit = 25) {
    try {
      const result = await this.makeRequest(`/content/${pageId}/child/comment?limit=${limit}`);
  ...
  async getSpacePages(spaceKey, limit = 50, start = 0) {
    try {
      const result = await this.makeRequest(`/content?spaceKey=${spaceKey}&limit=${limit}&start=${start}&type=page`);
  ...
  async getPageAttachments(pageId, limit = 25) {
    try {
      const result = await this.makeRequest(`/content/${pageId}/child/attachment?limit=${limit}`);
  ...
  async getMyPages(limit = 25, spaceKey = null) {
    try {
      ...
      let cql = `creator = "${user.username || user.accountId}" AND type = page`;
  ...
  async getPageTemplates(spaceKey) {
    try {
      const result = await this.makeRequest(`/content?spaceKey=${spaceKey}&type=template`);
          ...
          tools: [
            {
              ...
              inputSchema: {
                type: 'object',
                properties: {
                  limit: { type: 'number', default: 25, description: 'Maximum number of spaces to return' }
                }
              }
            },
            {
              name: 'get_page',
              description: 'Get a Confluence page by ID with clean text content',
              inputSchema: {
                type: 'object',
                properties: {
            ...
            },
            {
              ...
              inputSchema: {
                ...
                properties: {
                  ...
                  content: { type: 'string', description: 'HTML content of the page (storage format)' },
                  parentPageId: { type: 'string', description: 'ID of parent page (optional)' }
                },
            ...
            },
            {
              ...
              inputSchema: {
                ...
                properties: {
                  ...
                  content: { type: 'string', description: 'New HTML content of the page (storage format)' },
                  version: { type: 'number', description: 'Current version number of the page' }
                },
            ...
            },
            {
              ...
              inputSchema: {
                type: 'object',
                properties: {
                  pageId: { type: 'string', description: 'ID of the parent page' },
                  limit: { type: 'number', default: 25, description: 'Maximum number of child pages to return' }
                },
                required: ['pageId']
              }
            },
            {
              ...
              inputSchema: {
                ...
                properties: {
                  ...
                  limit: { type: 'number', default: 50, description: 'Maximum number of pages to return' },
                  start: { type: 'number', default: 0, description: 'Start index for pagination' }
                },
                required: ['spaceKey']
              }
            },
  ...
  async handleToolCall(params) {
    ...
    try {
      switch (name) {
        ...
        case 'search_pages':
          const searchResults = await this.searchPagesCompact(args.query, args.limit);
          return { content: [{ type: 'text', text: JSON.stringify(searchResults, null, 2) }] };

        case 'create_page':
          const createResult = await this.createPage(args.spaceKey, args.title, args.content, args.parentPageId);
          return { content: [{ type: 'text', text: JSON.stringify(createResult, null, 2) }] };

        case 'update_page':
          const updateResult = await this.updatePage(args.pageId, args.title, args.content, args.version);
        ...
        case 'get_page_history':
          const historyResult = await this.getPageHistory(args.pageId, args.limit);
          return { content: [{ type: 'text', text: JSON.stringify(historyResult, null, 2) }] };

        case 'get_child_pages':
          const childPages = await this.getChildPagesCompact(args.pageId, args.limit);
          return { content: [{ type: 'text', text: JSON.stringify(childPages, null, 2) }] };
        ...
        case 'add_page_comment':
          const commentResult = await this.addPageComment(args.pageId, args.comment);
        ...
        case 'get_space_pages':
          ...
          return { content: [{ type: 'text', text: JSON.stringify(spacePages, null, 2) }] };
        ...
        case 'add_page_label':
          ...
          return { content: [{ type: 'text', text: JSON.stringify(labelResult, null, 2) }] };
        ...
        case 'get_recent_pages':
          const recentPages = await this.getRecentPages(args.limit, args.spaceKey);
          return { content: [{ type: 'text', text: JSON.stringify(recentPages, null, 2) }] };

        case 'get_my_pages':
          const myPages = await this.getMyPages(args.limit, args.spaceKey);
          return { content: [{ type: 'text', text: JSON.stringify(myPages, null, 2) }] };

        case 'get_pages_by_label':
          const pagesByLabel = await this.getPagesByLabel(args.label, args.limit, args.spaceKey);
          return { content: [{ type: 'text', text: JSON.stringify(pagesByLabel, null, 2) }] };
        ...
        case 'get_space_details':
          ...
          return { content: [{ type: 'text', text: JSON.stringify(spaceDetails, null, 2) }] };
